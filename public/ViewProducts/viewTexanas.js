...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function activarListenersGaleriaModal() {
        ...
        const thumbnails = document.querySelectorAll('#img-texana .thumbnail-modal, #img-sombrero .thumbnail-modal'); // Seleccion genérica
...
function aplicarFiltros() {
    ...
    const coloresSeleccionados = Array.from(document.querySelectorAll('.check-color:checked')).map(cb => cb.value);
    ...
    fetch('/LaHerradura/Controller/CRUD_Texanas/FiltrarTexanas.php', {
        method: 'POST',
    ...
    .then(data => {
        ...
        data.forEach(producto => {
            const cardHTML = `
                <div class='card abrir-modal-vp' data-id='${producto.id_texana}'>
                    <div class='img-producto'>
                        <img src='/LaHerradura/uploads/texanas/${producto.Img1}' alt='${producto.Nombre}'>
                    </div>
...
document.querySelectorAll('.check-copa, .check-talla, .check-color, #filtro-precio-min, #filtro-precio-max').forEach(el => {
