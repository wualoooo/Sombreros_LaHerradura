...
try {
    ...
    if (empty($_POST['NombreHorma'])) {
        throw new Exception("El campo Nombre es obligatorio.");
    }

    $nombre = trim($_POST['NombreHorma']);
    ...
    // Insertar
    $sql = "INSERT INTO hormas (Nombre) VALUES (?)";
    ...
    if ($stmt->execute()) {
        ...
        $response['message'] = "¡Horma registrada con éxito!";
    } else {
        if ($conn->errno == 1062) {
            throw new Exception("Esa horma ya existe.");
        } else {
