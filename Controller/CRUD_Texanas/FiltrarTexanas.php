...
// 3. Filtro por Copas
if (!empty($data['copas'])) {
    $placeholders = implode(',', array_fill(0, count($data['copas']), '?'));
    $whereClausulas[] = "Copa IN ($placeholders)";
    foreach ($data['copas'] as $copaId) {
        $parametros[] = $copaId;
        $tipos .= "i";
    }
}
...
if (!empty($data['colores'])) {
    $placeholders = implode(',', array_fill(0, count($data['colores']), '?'));
    $whereClausulas[] = "Color IN ($placeholders)";
