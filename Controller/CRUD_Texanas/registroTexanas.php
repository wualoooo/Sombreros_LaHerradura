...
try {
    ...
    function generarMiniaturaBase64($archivo_temporal) {
        ...
        $ext = strtolower(pathinfo($_FILES['imgTexana1']['name'], PATHINFO_EXTENSION));
    ...
    $miniatura = generarMiniaturaBase64($_FILES['imgTexana1']['tmp_name']);
    ...
    $sql = "INSERT INTO texanas (SKU, Nombre, Color, Horma, Copa, Tam_Copa, Tam_ala, Material, Precio, Tallas, Img1, Img2, Img3, Img4, Miniatura, Estado) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)";
    ...
    if ($stmt->execute()) {
        ...
        $response['message'] = 'Texana e inventario registrados correctamente.';
    } else {
